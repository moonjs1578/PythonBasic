...
#   + index 사용(슬라이싱 가능) ex) [], [:]
#   + 정렬 가능
#   + mutable(최초 생성된 후 변경 가능)
#   + packing 과 unpacking 가능
#   + 함수: append(), extend(), insert(), remove(), pop(), index(), sorted(), 등등
...
print(b) # b.extend(c) print(b)== print(b+c)
#   라. remove() : 값으로 삭제
d=[1,2,3]
d.remove(2)
print(d)
...
#   + sorted() → 복제한 후 값을 정렬(원본 값 유지)  sort사용말고 sorted사용하기
...
print(intro.items())    # Key, Value만 추출
