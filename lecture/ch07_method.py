...
# - 함수 내용은 가능하면 짧게 작성(Code Line줄이기)
...
#           제곱기능(곱셉2번) → 곱셈() 2번 호출 → 제곱
# 3. 함수를 만들어야하는 경우
# - 공통적으로 사용되는 코드는 함수로 생성
# - 복잡한 로직이 사용되는 경우 실별 가능한 이름의 함수 생성

# ** 함수의 종류 **
# 1. 내장 함수(Built-in fuction)
...
#   나. import 호출 (ex: from datetime improt datetime)
...
# 2. 함수 호출
total=sum_two_value(5, 10) # 반환값이 있으면 함수가 반환값으로 바뀜
print(total)

# 3. 인자(입력 매개변수)
#   - 함수에 전달되는 입력값
#   - 함수 정의문과 호출문의 parameter 갯수가 동일해야함
#   - parameter로 int, str, float, boole, list 등 사용가능
#   - 심지어 사용자 정의 함수를 parameter로 전달 가능
#   - parameter값 2개 이상 사용 시 정의 된 순서대로 전달해야 함

# 4. Default Parameter
#   - 함수 호출 시 parameter를 전달받지 못한 경우 기본값 사용
#   def test(a,b,c=5):        #(O) parameter에서 변수 선언시 default값
...
num1 = 20
def test():
    global num1 # global 전역변수 지칭 키워드(global 사용 금지)
    num1=30
    
test()

# 7. 가변길이인자(Varialbe length parameter)
#   - *args() : tuple type
#   - **kwargs() : dict type
