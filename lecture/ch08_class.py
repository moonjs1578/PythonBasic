...
# 2. 객체나 클래스를 만들 필요 없이 바로 코딩 가능(프로그램 호출 쉽게 추적 가능)
...
# Class(힐스테이트 설계 도면) → 객세생성 → 수완동(수완 : 힐스테이트 : 인스턴스)
# Class(힐스테이트 설계 도면) → 객세생성 → 봉선동(봉선 : 힐스테이트 : 인스턴스)
# Class(힐스테이트 설계 도면) → 객세생성 → 지산동(지산 : 힐스테이트 : 인스턴스)

# 객체 사용 3단게
# 1. Class 작성
class ChosunTest:
    # 클래스에 소속된 멤버함수는 
    # 첫번째로 매개변수로 self인자를 반드시 받아야함!
    # self : 생성된 인스턴스(me)
...
# ※ 모데링 : 현실세계를 컴퓨터 세계 구현한 것!

# 현실세계(우체부)      →       컴퓨터세계(우체부Class)
#   우체부가                    class 우체부 : 
#   편지를                      누가 = 우체부
#   배달한다.                   무엇 = 편지
#                               def 배달한다(누가, 무엇) : 
