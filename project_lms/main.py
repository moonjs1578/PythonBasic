...
# 1-1. Streamlit 세션 상태에서 현재 페이지를 추적하기 위한 초기 설정
...
def main_page():
    ...
    with st.form("search_form"):
        ...
        if submitted:
            ...
            st.write(f"{keyword}로 검색된 결과는 총{len(rows)}입니다.")   
    ...
    if len(event.selection["rows"]):
        selected_idx=event.selection["rows"][0] #선택한  Dataframe의 Index값
        
        # 등록(insert)와 달리 수정(delete)과 삭제(DELETE)는
